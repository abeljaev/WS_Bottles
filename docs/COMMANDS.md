...
**Примечания:**
- Обнуляет соответствующий счётчик в ПЛК
- Вызывается когда процент заполнения достиг 100% и мешок заменён
...
| `cmd_weight_error_reset` | Сброс ошибки взвешивания |
| `cmd_reset_weight_reading` | Сброс показания весов |

**Формат:** Строка с именем команды, например `"cmd_full_clear_register"`

---

### Команды-заглушки (не реализованы)
...
| **Когда приходит** | Vision вернул "none", таймаут, или несовпадение с ПЛК |
...
**Поля:**
- `bottle_count` / `bank_count` — количество принятых контейнеров
- `bottle_fill_percent` / `bank_fill_percent` — процент заполнения мешка
